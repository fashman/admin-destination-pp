...
class List extends Component {
  render() {
    const { listData, fetchList, isloading, search, changeState, areaList } = this.props;
    ...
    const columns = [{
      ...
      dataIndex: 'zoneId',
      width: '8%',
      render: (text, record, index) => text && areaList.length ? areaList.find(v=>(
        v.id === text
      ))[`name${lang==='en'? 'En':'Cn'}`] : (lang==='en' ? 'Not Push Area' : '暂未投放')
    }, { // 店铺审核/经营状态
      ...
      render: (text, record, index) => (
        <div>
          <Popconfirm title={intl.get('operation.deleteMsg')} 
            onConfirm={()=>this.props.deleteMerchant(record.shopCode)}
            className="delete-modal"
          >
            <Button type='danger' style={{marginRight: 10}}>{intl.get('operation.delete')}</Button>
          </Popconfirm>
        ...
        />
        {
          resultList && resultList.length ? (
            <Pagination 
              ...
              showTotal={()=>lang === 'en' ? `Total ${total} items` : `共 ${total} 条记录`}
            />
          ) : null
        }
      </div>
