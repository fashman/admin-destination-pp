...
function* copyStore(action) {
  try {
    ...
    if (res.rs === 1) {
      ...
      const description = (
        <p>
          {intl.get('pages.storeList.copyModal.storeId')}：{res.data.shopCode}，
          <a href={`${Base.domain}/merchant/edit-page?id=${res.data.shopCode}`} target="_blank">{intl.get('pages.storeList.copyModal.goedit')}</a>
        </p>
      );
